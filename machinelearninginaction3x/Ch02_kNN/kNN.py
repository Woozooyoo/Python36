...
def classify0(inX, dataSet, labels, k):
    ...
    sqDiffMat = diffMat ** 2  # 距离公式  每一个cell元素的平方
    sqDistances = sqDiffMat.sum(axis=1) # 按y轴的平行线 相加
    distances = sqDistances ** 0.5  # 开平方得真正距离
    sortedDistIndicies = distances.argsort()  # 按距离从小到大排序其索引 [0 1 3 2]
    classCount = {}
    for i in range(k):
...
def autoNorm(dataSet):  # 归一化
    ...
    m = dataSet.shape[0]  # dataSet是几行几列的第一个参数 即几行:1000行
    normDataSet = dataSet - tile(minVals, (m, 1))  # 复制 m行 [0, 0, 0] 再减
    normDataSet = normDataSet / tile(ranges, (m, 1))  # 和最小的差值 除以(max-min的差值)element wise divide
